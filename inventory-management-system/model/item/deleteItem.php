	...
	if(isset($_POST['itemDetailsItemNumber'])){
		...
		if(!empty($itemNumber)){
			...
			if($itemStatement->rowCount() > 0){
				...
				echo '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>ទំនិញត្រូវបានលុប។</div>';
				exit();
				
			} else {
				// Item does not exist, therefore, tell the user that he can't delete that item 
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>ទំនិញមិនមាននៅក្នុងមូលដ្ឋានទិន្នន័យ (Database) ទេ។ ដូច្នោះហើយ មិនអាចលុបបានទេ។</div>';
				exit();
			}
